...
const kirbyphpmustacheConfig = {
  'class': '.kirby-php-mustache',
  'classIsActive': 'is-active'
};

$(function($) {

  let tabs = kirbyphpmustacheConfig.class + '__tab';
  let views = kirbyphpmustacheConfig.class + '__view';
  $(tabs).on( 'click', function(ev) {
    let $wrapper = $(ev.target).parents(kirbyphpmustacheConfig.class);
    $wrapper.find(tabs).removeClass(kirbyphpmustacheConfig.classIsActive);
    $wrapper.find(views).removeClass(kirbyphpmustacheConfig.classIsActive);
    $wrapper.find(ev.target).addClass(kirbyphpmustacheConfig.classIsActive);
    $wrapper.find('.' + $(ev.target).attr('data-tab')).addClass(kirbyphpmustacheConfig.classIsActive);
  });
  $(tabs + 's').children('*:first-of-type').each( function (idx, ele) { //
    $(ele).addClass(kirbyphpmustacheConfig.classIsActive);
    $('.' + $(ele).attr('data-tab')).addClass(kirbyphpmustacheConfig.classIsActive);
  });
  ...
  Rainbow.extend('html-style', [
    ...
    {
      name: 'regexp',
      pattern: /\#?\d+(px|em|%|rem|vh|vw|ch)?/gm
    },
    {
      name: 'support',
      pattern: /([\w\-])+:/gm
  ...
  Rainbow.color(function() {
    $(kirbyphpmustacheConfig.class).removeClass('hidden');
  });
});

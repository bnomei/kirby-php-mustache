...
class KirbyPHPMustache {
  ...
  static private 	$settings = [];    // plugin
  ...
  static private function instance($stringLoader = false) {
    ...
    if(self::is_closure($cd)) {
      self::$settings['cache_dir'] = $cd();
		...
		if(self::$settings['cache']) dir::make($cache_dir);
    ...
    if(c::get('plugin.mustache.helpers.globals', false)) {
      self::$helpers = array_merge(self::$helpers, [
		...
		self::$config['escape'] = c::get('plugin.mustache.escape',
			function($value) {
  ...
  static public function render($template, $data = null, $dump = false) {
    ...
    if($dump) {
      ...
      if(!f::exists($s)) $s = self::$settings['templates_dir'] . DS . '_'.str_replace($ext, '', $template).'.scss';

      if(f::exists($t)) $f = f::read($t);
      if(f::exists($p)) $f = f::read($p);
  ...
  static public function readFile($path) {
    if(!self::instance()) return null; // force load settings
  ...
  static public function listTemplates() {
    if(!self::instance()) return null; // force load settings
    ...
    foreach($folders as $folder) {
      foreach(dir::read($folder) as $file) {
        if(f::extension($file) != self::$settings['extension']) continue;
...
$kirby->set('page::method', 'mustache',
